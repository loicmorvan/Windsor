...
{
	/// <summary>Sets up framework level activators for Controllers, TagHelpers and ViewComponents and adds additional sub dependency resolvers</summary>
	/// <param name = "services">
	...
	/// <summary>For making types available to the <see cref = "IServiceCollection" /> using 'late bound' factories which resolve from Windsor. This makes things like the @Inject directive in Razor work.</summary>
	...
	private static void AddApplicationComponentsToWindsor(IWindsorContainer container, WindsorRegistrationOptions options)
	{
		...
		if (!options.TagHelperAssemblyRegistrations.Any() && !options.TagHelperComponentRegistrations.Any()) container.Register(Classes.FromAssemblyInThisApplication(options.EntryAssembly).BasedOn<ITagHelper>().LifestyleScoped());
		...
		foreach (var controllerRegistration in options.ControllerComponentRegistrations) container.Register(controllerRegistration);

		foreach (var tagHelperRegistration in options.TagHelperComponentRegistrations) container.Register(tagHelperRegistration);

		foreach (var viewComponentRegistration in options.ViewComponentComponentRegistrations) container.Register(viewComponentRegistration);
	}
	...
	private static IServiceProvider InitialiseFrameworkServiceProvider(IServiceCollection services, Func<IServiceProvider> serviceProviderFactory, IWindsorContainer container)
	{
		...
		foreach (var acceptServiceProvider in container.ResolveAll<IAcceptServiceProvider>()) acceptServiceProvider.AcceptServiceProvider(serviceProvider);
