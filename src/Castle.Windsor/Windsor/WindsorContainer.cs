	...
	IWindsorContainer
{
	...
	private readonly Dictionary<string, IWindsorContainer> childContainers = new(StringComparer.OrdinalIgnoreCase);
	private readonly object childContainersLocker = new();
	...
	/// <summary>
	///     Initializes a new instance of the <see cref = "WindsorContainer" /> class using a resource pointed to by the parameter. That may be a file, an assembly embedded resource, a UNC path or a config
	///     file section.
	...
	{
		if (Installer != null) Installer.SetUp(this, kernel.ConfigurationStore);
	}
	...
	///     <see
	///         cref = "FromAssembly" />
	///     class to automatically instantiate and run your installers. You can also use <see cref = "Configuration" /> class to install components and/or run aditional installers specofied in a
	///     configuration file.
	/// </remarks>
	...
	/// <summary>
	///     Registers the components with the <see cref = "IWindsorContainer" />. The instances of <see cref = "IRegistration" /> are produced by fluent registration API. Most common entry points are
	...
	/// <summary>Removes (unregisters) a subcontainer.  The components exposed by this container will no longer be accessible to the child container.</summary>
	/// <param name = "childContainer"></param>
	...
	/// <summary>Resolve all valid components that match this type.
	///     <typeparam name = "T">The service type</typeparam>
