...
public class DefaultTypedFactoryComponentSelector : ITypedFactoryComponentSelector
{
	...
	/// <param name = "fallbackToResolveByTypeIfNameNotFound">
	...
	protected DefaultTypedFactoryComponentSelector() : this(true)
	{
	}

	/// <summary>If set to <c>true</c>, will fallback to resolving by type, if can not find component with specified name. This property is here for backward compatibility. It is recommended not to use it.</summary>
	protected bool FallbackToResolveByTypeIfNameNotFound { get; set; }

	/// <summary>If set to <c>true</c>, all methods with names like 'GetSomething' will try to resolve by name component 'something'.</summary>
	...
	/// <summary>
	///     Builds <see cref = "TypedFactoryComponentResolver" /> for given call. By default if <paramref name = "componentType" /> is a collection returns factory calling
	///     <see cref = "IKernel.ResolveAll(System.Type)" /> on collection's item type, otherwise standard <see cref = "TypedFactoryComponentResolver" />.
	/// </summary>
	...
	/// <summary>
	///     Selects arguments to be passed to resolution pipeline. By default passes all given <paramref name = "arguments" /> keyed by names of their corresponding <paramref name = "method" />
	///     parameters.
	/// </summary>
	/// <param name = "method"></param>
	...
	protected virtual Arguments GetArguments(MethodInfo method, object[] arguments)
	{
		...
		for (var i = 0; i < parameters.Length; i++) argumentMap.Add(parameters[i].Name, arguments[i]);
		return argumentMap;
	}

	/// <summary>Selects name of the component to resolve. If <paramref name = "method" /> Name is GetFoo returns "Foo", otherwise <c>null</c>.</summary>
	/// <param name = "method"></param>
	/// <param name = "arguments"></param>
	/// <returns></returns>
	protected virtual string GetComponentName(MethodInfo method, object[] arguments)
	{
		...
		if (GetMethodsResolveByName && method.Name.StartsWith("Get", StringComparison.OrdinalIgnoreCase)) componentName = method.Name.Substring("Get".Length);
		return componentName;
	}

	/// <summary>Selects type of the component to resolve. Uses <paramref name = "method" /> return type.</summary>
	/// <param name = "method"></param>
	/// <param name = "arguments"></param>
	/// <returns></returns>
