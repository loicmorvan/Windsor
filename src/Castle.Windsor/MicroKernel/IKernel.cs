...
/// <summary>The <c>IKernel</c> interface exposes all the functionality the MicroKernel implements.</summary>
/// <remarks>
///     It allows you to register components and request them by their name or the services they expose. It also allow you to register facilities and subsystem, thus augmenting the functionality exposed
///     by the kernel alone to fit your needs. <seealso cref = "IFacility" /> <seealso cref = "ISubSystem" />
/// </remarks>
public partial interface IKernel : IKernelEvents, IDisposable
{
	/// <summary>Returns the implementation of <see cref = "IComponentModelBuilder" /></summary>
	...
	/// <summary>Gets or sets the implementation of <see cref = "IProxyFactory" /> allowing different strategies for proxy creation.</summary>
	IProxyFactory ProxyFactory { get; set; }

	/// <summary>Gets or sets the implementation for <see cref = "IReleasePolicy" /></summary>
	IReleasePolicy ReleasePolicy { get; set; }

	/// <summary>Returns the implementation for <see cref = "IDependencyResolver" /></summary>
	IDependencyResolver Resolver { get; }

	/// <summary>Support for kernel hierarchy</summary>
	/// <param name = "kernel"></param>
	void AddChildKernel(IKernel kernel);

	/// <summary>Adds a <see cref = "IFacility" /> to the kernel.</summary>
	/// <param name = "facility"></param>
	/// <returns></returns>
	IKernel AddFacility(IFacility facility);

	/// <summary>Creates and adds an <see cref = "IFacility" /> facility to the kernel.</summary>
	...
	/// <summary>Register a new component resolver that can take part in the decision making about which handler(s) to resolve and in which order</summary>
	void AddHandlersFilter(IHandlersFilter filter);

	/// <summary>Adds (or replaces) an <see cref = "ISubSystem" /></summary>
	...
	/// <summary>Returns an implementation of <see cref = "ISubSystem" /> for the specified name. <seealso cref = "SubSystemConstants" /></summary>
	/// <param name = "name"></param>
	/// <returns></returns>
	...
	///     <see cref = "Component.For{TService}" /> method to register a single type or (recommended in most cases) <see cref = "Classes.FromAssembly(Assembly)" />. Let the Intellisense drive you through
