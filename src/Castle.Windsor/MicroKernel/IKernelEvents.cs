...
public interface IKernelEvents
{
	...
	/// <summary>Event fired when a dependency is being resolved, it allows the dependency to be changed, but the client ComponentModel must not be changed.</summary>
	event DependencyDelegate DependencyResolving;

	/// <summary>
	///     Event fired when registration / installation process is completed. That is when container is about to exit<see cref = "IKernel.Register" /> method. This event is raised once regardless of how
	///     many components were registered. If the <see cref = "IKernel.Register" /> is called by <see cref = "IWindsorContainer.Install" /> the event is raised when that method exits.
	/// </summary>
	event EventHandler RegistrationCompleted;

	/// <summary>
	///     Event fired when a collection is being resolved (via <see cref = "IKernel.ResolveAll(System.Type)" /> or another overload) and the collection is empty. Implementors would usually log that fact or
