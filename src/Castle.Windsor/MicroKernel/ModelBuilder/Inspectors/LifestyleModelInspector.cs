...
public class LifestyleModelInspector(IConversionManager converter) : IContributeComponentModelConstruction
{
	...
	{
		if (!ReadLifestyleFromConfiguration(model)) ReadLifestyleFromType(model);
	}
			...
			switch (lifestyleType)
			{
				...
				default:
					throw new InvalidOperationException(string.Format(
						"Component {0} has {1} lifestyle. This is not a valid value.", model.Name, lifestyleType));
			}
		}
	...
	protected virtual void ValidateTypeFromAttribute(Type typeFromAttribute, Type expectedInterface, string attribute)
	{
		...
		throw new InvalidOperationException(string.Format(
			"The Type '{0}' specified in the '{2}' attribute must implement {1}", typeFromAttribute.FullName,
			expectedInterface.FullName, attribute));
	}
	...
	private Type GetMandatoryTypeFromAttribute(ComponentModel model, string attribute, LifestyleType lifestyleType)
	{
		...
		if (rawAttribute == null)
			throw new InvalidOperationException(string.Format(
				...
				model.Name, lifestyleType, attribute));
	...
	private bool IsBindMethod(MemberInfo methodMember, object _)
	{
		...
		if (method.ReturnType != typeof(IHandler)) return false;
		var parameters = method.GetParameters();
		if (parameters.Length != 1) return false;
		if (parameters[0].ParameterType != typeof(IHandler[])) return false;
		return true;
	}
}