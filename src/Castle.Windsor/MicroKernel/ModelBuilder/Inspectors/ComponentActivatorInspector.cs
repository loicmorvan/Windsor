...
/// <remarks>
///     This inspector is not guarantee to always set up an component activator type. If nothing could be found it wont touch the model. In this case is up to the kernel to establish a default component
...
public class ComponentActivatorInspector : IContributeComponentModelConstruction
{
	...
	{
		if (!ReadComponentActivatorFromConfiguration(model)) ReadComponentActivatorFromType(model);
	}

	/// <summary>Reads the attribute "componentActivatorType" associated with the component configuration and verifies it implements the <see cref = "IComponentActivator" /> interface.</summary>
