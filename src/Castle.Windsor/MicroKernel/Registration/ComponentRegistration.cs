    ...
    where TService : class
{
    ...
    ///     <see cref="Property" /> and <see cref="Parameter" />. Use the static
    ...
    /// <summary>
    ///     Adds <typeparamref name="TService2" /> and <typeparamref name="TService3" /> as additional services to be
    ...
    /// <summary>
    ///     Adds <typeparamref name="TService2" /> , <typeparamref name="TService3" /> and
    ///     <typeparamref name="TService4" /> as additional services to be exposed by this component.
    /// </summary>
    /// <typeparam name="TService2"> The first forwarded type. </typeparam>
    /// <typeparam name="TService3"> The second forwarded type. </typeparam>
    ...
    ///     <typeparamref name="TService5" /> as additional services to be exposed
    ///     by this component.
    /// </summary>
    /// <typeparam name="TService2"> The first forwarded type. </typeparam>
    /// <typeparam name="TService3"> The second forwarded type. </typeparam>
    ...
    ///     will be used to access scope for the component. Otherwise the default scope
    ...
    ///     scope for current component's instances. The delegate will be invoked when current
    ///     component is about to be resolved and will be passed set of handlers to components higher up the resolution stack.
    ///     It ought to return one which it designages as the root which shall scope the
    ...
    /// <summary>
    ///     Sets component lifestyle to pooled. If <paramref name="initialSize" /> or <paramref name="maxSize" /> are not
    ...
    ///     be able to resolve some specific ones. Then you'd provide the name so that
    ...
    ///     provided by the user, but autogenerated and user has no interest in seing this name,
    ///     for example in diagnostics reports. Set a name of this registration. This is required if you have multiple
    ///     components for a given service and want to be able to resolve some specific ones. Then
    ///     you'd provide the name so that Windsor knows which one of the bunch you know. Otherwise don't bother setting the
    ///     name.
    ///     <para />
    ///     If not set, the <see cref="Type.FullName" /> of the <see cref="Implementation" /> will be used as the key to
    ///     register the component.
    ...
    /// <summary>
    ///     Stores a set of <see cref="LifecycleActionDelegate{T}" /> which will be invoked when the component is created
    ///     and before it's returned from the container.
    /// </summary>
    /// <param name="actions">
    ...
    ///     which means when it's released or it's lifetime scope ends. Notice that usage of
    ...
    /// <summary>
    ///     Services that are already present in the container will be skipped. If no new service is left the registration
        ...
        where TImpl : TService
    {
        ...
        {
            ExtendedProperties(Property.ForKey("factory.managedExternally").Eq(true));
        }
    ...
    ///     <paramref name="serviceFilter" /> allows user to narrow down the number of services
    ...
    ///     another, non-fallback, component will be registered exposing any of the same
    ///     services as this component, that other component will take precedence over this one, regardless of order in which
    ///     they are registered
    /// </summary>
    /// <returns> </returns>
    ...
    /// <summary>
    ///     Filters (settable) properties of the component's implementation type to expose in the container and specifies
    ///     if matched properties are considered mandatory.
    /// </summary>
    /// <param name="filter">
    ///     Rules for deciding whether given properties are exposed in the container or ignored and if they are mandatory, that
    ///     is Windsor will only successfully resole the component if it can provide value
