...
public sealed class BasedOnDescriptor : IRegistration
{
    ...
    /// <summary>
    ///     Allows customized configurations of each matching component with an implementation type that is assignable to
    ...
    /// <summary>
    ///     Allows customized configurations of each matching component that satisfies supplied
    ///     <paramref name="condition" /> and alternative configuration for the rest of components.
    /// </summary>
    /// <param name="condition"> Condition to satisfy </param>
    ...
    /// <summary>
    ///     Uses the first interface of a type. This method has non-deterministic behavior when a type implements more
    ///     than one interface!
    /// </summary>
    /// <returns> </returns>
    ...
    /// <summary>
    ///     Uses <paramref name="implements" /> to look up the sub interface. For example, if you have IService and
    ...
    ///     services and do not want to specify all of them.
    /// </summary>
    ...
    {
        return _ifFilter == null || _ifFilter.GetInvocationList().Cast<Predicate<Type>>().All(filter => filter(type));
    }

    private bool ExecuteUnlessCondition(Type type)
    {
        return _unlessFilter != null &&
    ...
    internal bool TryRegister(Type type, IKernel kernel)
    {
        ...
        _configuration?.Invoke(registration);
