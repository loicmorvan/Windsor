...
public class LazyComponentsTestCase : AbstractContainerTestCase
{
    ...
    public void Can_resolve_lazy_component_requiring_arguments_inline()
    {
        ...
        Assert.Throws<DependencyResolverException>(() => missingArguments.Value);
    ...
    private static LazyThreadSafetyMode GetMode(Lazy<A> lazy)
    {
        return (LazyThreadSafetyMode)(lazy?.GetType()
                                          .GetProperty("Mode", BindingFlags.NonPublic | BindingFlags.Instance)
                                          ?.GetValue(lazy, null) ??
