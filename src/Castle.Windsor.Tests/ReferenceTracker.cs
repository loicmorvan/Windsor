...
public static class ReferenceTracker
{
	...
	///     <para>
	///         (If the expression is evaluated directly inside the test method, the runtime will keep it alive until the test method returns if the runtime is in debug mode. ReSharper’s test runner runs in
...
public struct ReferenceTracker<T> where T : class
{
	...
	/// <summary>
	///     <para>Calls <see cref = "GC.Collect()" /> and asserts that the tracked instance is still alive and passes it to the specified action.</para>
	///     <para>
	///         Be careful not to let the tracked instance become reachable via a local variable in the test method or it will be kept alive until the end of the test method if the runtime is in debug mode.
	///         (See <see cref = "Track" />.)
	///     </para>
	/// </summary>
	...
	/// <summary>
	///     <para>Calls <see cref = "GC.Collect()" /> and asserts that the tracked instance is still alive, passes it to the specified function, and returns the value returned by the specified function.</para>
	///     <para>
	///         Be careful not to let the tracked instance become reachable via a local variable in the test method or it will be kept alive until the end of the test method if the runtime is in debug mode.
	///         (See <see cref = "Track" />.)
	///     </para>
