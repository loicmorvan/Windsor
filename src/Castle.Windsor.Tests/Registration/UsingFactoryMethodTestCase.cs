...
public class UsingFactoryMethodTestCase : AbstractContainerTestCase
{
    ...
    public void Factories_returning_proxies_with_no_target_are_not_supported()
    {
        ...
        Assert.Equal(
            @"Can not apply commission concerns to component Late bound Castle.Windsor.Tests.Components.ICameraService because it appears to be a target-less proxy. Currently those are not supported.",
            exception.Message);
    }
    ...
    public void Proxying_type_with_no_default_ctor_throws_helpful_message()
    {
        ...
        var expected =
            "Can not instantiate proxy of class: Castle.Windsor.Tests.Config.Components.ClassWithConstructors." +
            Environment.NewLine +
